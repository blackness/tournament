...
import {
  handleMatchCompletionForOFSAA,
  rebuildAfterStandingsFinalize,
} from '../services/tournaments/ofsaaUltimateService'

const SCREEN = { PRE: 'pre', LIVE: 'live', POST: 'post' }
const PLAYER_PICKER_TIMEOUT = 10000
...
export function ScorekeeperPage() {
    ...
    async function load() {
      const { data: m } = await supabase
        ...
        .select(`
          ...
          tournament:tournaments(id, name, slug, sport_template_id, allow_ties,
      ...
      if (m.status === 'in_progress') {
        ...
        if (m.started_at) setGameStartTime(new Date(m.started_at).getTime())
  ...
  async function loadRosters(m) {
    ...
    const [{ data: rA }, { data: rB }] = await Promise.all([
      supabase
        ...
        .eq('tournament_team_id', teamAId ?? 'x')
        .order('number'),
      supabase
        .from('tournament_players')
        .select('id, name, number')
        .eq('tournament_team_id', teamBId ?? 'x')
        .order('number'),
    ])
  ...
  async function endGame(forceTie = false) {
    ...
    const allowTies = match?.tournament?.allow_ties === true
  ...
  async function handleStatTap(teamId, statId) {
    ...
    if (statDef.adds_to_score && statId !== 'callahan') {
      // assist picker handled after primary player is picked
    }
  }
  ...
  async function handlePlayerPick(playerId) {
    if (pickerTimeout) {
...
function LiveScoringScreen({ match, events, roster, sportConfig, allowTies, onStat, onUndo, onSetCap, onConfirmEnd, gameStartTime }) {
      ...
      <div className="flex gap-2 px-3 pb-4 pt-2 border-t border-gray-800 flex-shrink-0">
        ...
        {!pendingEnd ? (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6, alignItems:'flex-end' }}>
            {!allowTies && match.score_a === match.score_b && (
              <p style={{ fontSize: 12, color: '#f87171', textAlign: 'right' }}>
                Ties are not allowed. Record another point before ending the game.
              </p>
            )}
    ...
    <div className="flex items-center gap-2 py-0.5">
      ...
      <span className="text-xs text-[var(--text-muted)] flex-1 truncate">
        ...
        {ev.score_a_after !== null && (
          <span className="ml-1 text-[var(--text-secondary)] tabular-nums">
      ...
      <div className="relative bg-[var(--bg-base)] border-t border-[var(--border-mid)] rounded-t-2xl px-4 pt-4 pb-8 max-h-[70vh] flex flex-col">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm font-semibold text-[var(--text-primary)]">
              {teamName} -- who {isSecondary ? 'assisted?' : 'got the ' + label.toLowerCase() + '?'}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 mb-3">
          ...
          {players.length === 0 && !addMode && (
            <p className="text-sm text-[var(--text-secondary)] text-center py-3 italic">
              No roster -- add players below or skip
            </p>
          )}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:"0 16px" }}>
      <div className="bg-[var(--bg-base)] border border-[var(--border-mid)] rounded-2xl p-5 w-full max-w-sm space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-[var(--bg-raised)] text-[var(--text-muted)] text-sm">
            Cancel
          </button>
          <button
            onClick={() => name.trim() && onAdd(teamId, name.trim(), num.trim())}
            disabled={!name.trim()}
...
function ScoreTeam({ team, score, right }) {
  return (
    <div className={'flex items-center gap-3 ' + (right ? 'flex-row-reverse' : '')}>
      <div
        className="w-10 h-10 rounded-full flex items-center justify-center font-black text-lg border-2 border-white/10"
  ...
  return (
    <div className={'flex items-center gap-2 ' + (missing ? 'opacity-40' : '')}>
      <div
        ...
        style={{ backgroundColor: missing ? '#6b7280' : (team?.primary_color ?? '#6b7280') }}
      />
      ...
      {[match.team_a, match.team_b].map(team => (
        <div key={team?.id} className="flex flex-col items-center gap-3">
          ...
          <button
            ...
            style={{
              borderColor: team?.primary_color ?? '#6b7280',
              color: team?.primary_color ?? '#fff',
              backgroundColor: (team?.primary_color ?? '#6b7280') + '22',
            }}
          >
            +
          </button>
