          ...
          <div style={{ display:'flex', gap:0, marginTop:4, borderTop:'1px solid var(--border)', overflowX:'auto' }}>
            {TABS.map(([tab, label, href]) => (
              <button key={tab} onClick={() => {
                ...
                if (tab === 'standings' && divisions.length === 1) { navigate('/t/' + slug + '/standings/' + divisions[0].id); return }
                if (tab === 'standings' && divisions.length > 1) { setActiveTab('standings'); return }
                setActiveTab(tab)
              }}
...
function LiveMatchCard({ match: m }) {
  ...
  return (
    <div role="link" tabIndex={0}
      ...
      style={{ display:'block', background:'var(--bg-raised)', border:'1px solid var(--border)', borderRadius:14, padding:'18px 20px', cursor:'pointer', transition:'border-color 0.15s' }}
        ...
        {m.venue?.youtube_url ? (
          <Link to={'/watch/' + m.id}
            style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, fontWeight:700,
              color:'#fff', textDecoration:'none', padding:'4px 12px', borderRadius:20,
