...
export function TeamPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const matchIds = (m ?? []).map(mx => mx.match_id)
  ...
  const scheduledMatches = matches.filter(m => !m.is_projected)
  const projectedMatches = matches.filter(m => m.is_projected)
    ...
    <div style={{ maxWidth:640, margin:'0 auto', padding:'32px 20px 80px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:6, flexShrink:0 }}>
          <button
            onClick={() => setFaved(toggleFavorite(teamId).includes(teamId))}
          ...
          {divTeams.length > 0 && (
            <div style={{ position:'relative' }} className="group">
              ...
              >
                <p style={{ padding:'10px 14px 6px', fontSize:10, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--text-muted)' }}>
                  Compare with
                </p>
          ...
          {matches.length === 0 ? (
            ...
            <>
              {scheduledMatches.length > 0 && (
                <div>
                  <h3 style={{ fontSize:12, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--text-muted)', marginBottom:8 }}>
                    Scheduled Games
                  </h3>
                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                    {scheduledMatches.map(m => <MatchCard key={m.match_id} match={m} />)}
                  </div>
                </div>
              )}
              ...
              {projectedMatches.length > 0 && (
                <div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                    {projectedMatches.map(m => <MatchCard key={m.match_id} match={m} />)}
                  </div>
                </div>
              )}
            </>
        ...
        <div>
            ...
            <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden' }}>
              {players.map((p, i) => (
                <div key={p.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 16px', borderTop: i > 0 ? '1px solid rgba(42,42,50,0.5)' : 'none' }}>
                  ...
                  {!p.is_eligible && (
                    <span style={{ fontSize:10, fontWeight:700, color:'#f87171', background:'rgba(239,68,68,0.1)', padding:'2px 8px', borderRadius:20 }}>
                      Ineligible
                    </span>
                  )}
                </div>
          ...
          {team.head_coach_name && (
            <p style={{ fontSize:13, color:'var(--text-muted)', marginTop:12 }}>
              <span style={{ fontWeight:600, color:'var(--text-secondary)' }}>Coach:</span> {team.head_coach_name}
            </p>
          )}
        </div>
      )}
  ...
  return (
    <Link
      ...
      style={{
        ...
        gap:12,
        padding:'12px 14px',
        background: m.is_projected ? 'var(--bg-raised)' : 'var(--bg-surface)',
        border:`1px solid ${
          isLive
            ? 'rgba(34,197,94,0.25)'
            : m.is_projected
      ...
      onMouseEnter={e => {
        e.currentTarget.style.borderColor = m.is_projected ? 'rgba(245,158,11,0.45)' : 'var(--border-mid)'
      ...
      onMouseLeave={e => {
        e.currentTarget.style.borderColor =
          isLive
            ...
            ? 'rgba(245,158,11,0.28)'
            : 'var(--border)'
        e.currentTarget.style.background = m.is_projected ? 'var(--bg-raised)' : 'var(--bg-surface)'
      }}
    >
      <div style={{ width:72, textAlign:'center', flexShrink:0 }}>
        {m.scheduled_start
          ? <p style={{ fontFamily:'DM Mono, monospace', fontSize:11, color:'var(--text-muted)' }}>{formatTime(m.scheduled_start)}</p>
          : <p style={{ fontSize:11, color:'var(--text-muted)' }}>TBD</p>
        }

        {isLive && <p style={{ fontSize:10, fontWeight:700, color:'var(--live)' }}>LIVE</p>}

        {m.is_projected && !isLive && !isDone && (
          <p style={{ fontSize:10, fontWeight:700, color:'#f59e0b' }}>PROJECTED</p>
        )}

        {isDone && (
          <p style={{ fontSize:11, fontWeight:700, color: m.team_won ? '#4ade80' : '#f87171' }}>
            {m.team_won ? 'W' : 'L'}
          </p>
        )}
      </div>
      ...
      {(isDone || isLive) && m.team_score !== null && (
        <span
          style={{
            fontFamily:'DM Mono, monospace',
            fontSize:14,
            fontWeight:700,
            color: m.team_won ? '#4ade80' : 'var(--text-muted)',
            flexShrink:0
          }}
        >
          {m.team_score}-{m.opp_score}
        </span>
      )}
...
function PlayerStatsTable({ playerStats, players }) {
  ...
  return (
    <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden' }}>
              ...
              {STAT_COLS.map(col => (
                <th
                  ...
                  style={{
                    ...
                    background: sortBy === col.key ? 'var(--accent-dim)' : 'transparent',
                    width:36
                  }}
                >
                  {col.label}
      ...
      <p style={{ padding:'8px 14px', fontSize:11, color:'var(--text-muted)', borderTop:'1px solid var(--border)' }}>
