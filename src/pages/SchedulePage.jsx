...
export function SchedulePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: d } = await supabase.from('divisions')
        .select('id, name')
        .eq('tournament_id', t.id)
  ...
  if (filterVenue !== 'all') {
    filtered = filtered.filter(m => m.venue?.id === filterVenue)
  }

  if (filterDivision !== 'all') {
    filtered = filtered.filter(m => m.division?.id === filterDivision)
  }
  ...
  if (search.trim()) {
    ...
    filtered = filtered.filter(m =>
      ...
      m.round_label?.toLowerCase().includes(q) ||
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg-base)' }}>
      ...
      <div style={{ maxWidth:700, margin:'0 auto', padding:'16px 20px' }}>
        ...
        <div style={{ marginBottom:12 }}>
          <input
            type="text"
            placeholder="Search by team, label, or match code..."
            value={search}
  ...
  return (
    <div style={{ position:'relative' }}>
      ...
      <Link
        ...
        onMouseEnter={e => {
          e.currentTarget.style.borderColor = isLive ? 'rgba(34,197,94,0.5)' : 'var(--border-mid)'
          e.currentTarget.style.background = 'var(--bg-raised)'
        }}
        onMouseLeave={e => {
          e.currentTarget.style.borderColor = isLive ? 'rgba(34,197,94,0.3)' : 'var(--border)'
...
function TeamPill({ team }) {
  if (!team?.id) {
    return (
      <span style={{ fontSize:13, color:'var(--text-muted)', fontStyle:'italic', flex:1 }}>
        TBD
      </span>
    )
  }
...
function groupByTime(matches, reverse = false) {
  ...
  for (const m of matches) {
    ...
    if (!groups[key]) {
      groups[key] = {
        key,
        label: key === 'unscheduled' ? 'Unscheduled' : formatGroupTime(new Date(start)),
        matches: [],
      }
    }
