  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 20px 80px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <Link to={'/t/' + slug} style={{ color: 'var(--text-muted)' }}>
          <ChevronLeft size={20} />
        </Link>
      ...
      {live.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {live.map(m => (
              <GameCard key={m.id} match={m} isLive />
            ))}
          </div>
        </div>
      )}
...
function GameCard({ match: m, isLive = false }) {
  ...
  const time = m.time_slot?.scheduled_start
    ? new Date(m.time_slot.scheduled_start)
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        {(specialHighlight || bracketBadge || (!teamA?.id || !teamB?.id)) && (
      ...
      <Link
        ...
        style={{
          ...
          cursor: 'pointer',
          textDecoration: 'none',
          borderTop: `1px solid ${specialHighlight ? specialHighlight.border : 'var(--border)'}`,
...
function TeamPill({ team }) {
  if (!team?.id) {
    return (
      <span style={{ fontSize: 13, color: '#fde047', fontStyle: 'italic', flex: 1 }}>
        TBD
      </span>
    )
  }
...
function formatSeedName(team) {
  if (!team) return 'TBD'
  const base = team.short_name ?? team.name ?? 'TBD'
