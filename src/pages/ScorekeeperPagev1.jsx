...
export function ScorekeeperPage() {
  ...
  // Realtime: keep scores in sync if another device updates
  useEffect(() => {
    if (!matchId) return
    const ch = supabase.channel('sk-match-' + matchId)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'matches', filter: 'id=eq.' + matchId }, payload => {
        const n = payload.new
        setMatch(prev => prev ? { ...prev, score_a: n.score_a, score_b: n.score_b, status: n.status, winner_id: n.winner_id, cap_status: n.cap_status } : prev)
        if (n.status === 'complete' || n.status === 'forfeit') setScreen(SCREEN.POST)
      })
      .subscribe()
  ...
  async function handleStatTap(teamId, statId) {
    ...
    if (statDef.is_player_stat !== false) {
      showPicker({
        ...
        eventId: event.id,
        isSecondary: false,
        showAssistAfter: statDef.adds_to_score && statId !== 'callahan',
      })
    } else if (statDef.adds_to_score && statId !== 'callahan') {
      // No player stat but still ask for assist
      showPicker({
        statId: 'assist',
        teamId,
        label: 'Assist',
        eventId: event.id,
        isSecondary: true,
        showAssistAfter: false,
      })
    }
  }

  function showPicker({ statId, teamId, label, eventId, isSecondary, showAssistAfter = false }) {
    // Clear any existing timeout
    if (pickerTimeout) clearTimeout(pickerTimeout)

    setPicker({ statId, teamId, label, eventId, isSecondary, showAssistAfter })
  ...
  async function handlePlayerPick(playerId, playerName) {
    ...
    if (picker && playerId) {
      ...
      if (picker.isSecondary) {
        ...
        // If this was a goal, auto-show assist picker next
        if (picker.showAssistAfter) {
          setTimeout(() => {
            showPicker({
              statId:           'assist',
...
function LiveScoringScreen({ match, events, roster, sportConfig, onStat, onUndo, onSetCap, onConfirmEnd, gameStartTime }) {
  ...
  const lastEvents = [...events].reverse().slice(0, 3)
  ...
  const otherStats    = stats.filter(s => !s.adds_to_score && !s.is_negative && s.category !== 'defense' && s.id !== 'assist')
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {/* Stat grid */}
      <div className="flex-1 overflow-hidden px-3 py-3" style={{ overflowY: "auto" }}>
        {stats.length === 0 ? (
      ...
      {/* Recent events feed */}
      {lastEvents.length > 0 && (
        <div className="bg-[var(--bg-base)]/80 border-t border-gray-800 px-3 py-1 flex-shrink-0">
          {lastEvents.map(ev => (
