...
export function WatchPage() {
  ...
  function flashTicker(ev) {
    ...
    const scoreStr =
      ev.score_a_after !== null && ev.score_b_after !== null
        ? ` - ${ev.score_a_after}-${ev.score_b_after}`
        : ''

    setTicker({
      text: playerName ? `${playerName} - ${label}${scoreStr}` : `${label}${scoreStr}`,
      color: ev.team?.primary_color ?? '#8b5cf6',
    })
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'game_events', filter: 'match_id=eq.' + matchId },
        async payload => {
  ...
  const watchUrl = toYouTubeWatchUrl(rawUrl)
      ...
      <div style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: '#000', aspectRatio: '16/9', width: '100%', position: 'relative', flexShrink: 0 }}>
          {embedUrl ? (
            ...
            <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 13, gap: 12 }}>
        ...
        <div style={{ background: '#0f0f12', padding: '14px 20px', display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, textAlign: 'center' }}>
            {clockStr ? (
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 22, fontWeight: 600, color: '#fff', letterSpacing: '0.05em' }}>
                {clockStr}
              </div>
            ) : isDone ? (
              <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.1em' }}>FINAL</div>
            ) : (
        ...
        {watchUrl && (
          <div style={{ padding: '10px 16px', background: '#0a0a0c', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <a
              href={watchUrl}
              target="_blank"
              rel="noreferrer"
    ...
    <div style={{ width: '100vw', height: '100vh', background: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 10 }}>
          {clockStr ? (
            <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 18, fontWeight: 600, color: '#fff', letterSpacing: '0.06em' }}>
              {clockStr}
            </span>
          ) : isDone ? (
          ...
          {isLive && (
            <div style={{ fontSize: 8, fontWeight: 700, color: '#22c55e', letterSpacing: '0.12em', marginTop: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
      ...
      <div onMouseEnter={() => setDrawerOpen(true)} style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 20, zIndex: 19 }} />
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexDirection: right ? 'row-reverse' : 'row', flex: 1 }}>
      ...
      <div style={{ textAlign: right ? 'right' : 'left' }}>
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 80 }}>
...
function MobileEventRow({ event: ev }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      {isScore && (
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.7)', flexShrink: 0 }}>
          {ev.score_a_after}-{ev.score_b_after}
        </span>
      )}
    </div>
...
function DrawerEventRow({ event: ev }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      {isScore && (
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: 'rgba(255,255,255,0.5)', flexShrink: 0 }}>
          {ev.score_a_after}-{ev.score_b_after}
        </span>
      )}
    </div>
...
function toYouTubeWatchUrl(url) {
  ...
  try {
    ...
    if (u.hostname.includes('youtube.com')) {
      ...
      if (u.pathname.startsWith('/live/')) return url
    }
