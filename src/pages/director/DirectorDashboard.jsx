...
import { Plus, Trash2, AlertTriangle, X, ChevronRight, Calendar, Users } from 'lucide-react'
...
export function DirectorDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await db.tournaments.mine(user.id)
      const tournamentList = data ?? []
      setTournaments(tournamentList)

      const counts = {}
      for (const t of tournamentList) {
        const { data: activeNow } = await supabase.rpc('fn_active_session_count', {
          p_tournament_id: t.id,
          p_minutes: 5,
        })
        counts[t.id] = activeNow ?? 0
      }
  ...
  const formatDate = d =>
    d
      ? new Date(d + 'T12:00').toLocaleDateString('en-CA', {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })
              ...
              >
                <div
                  style={{
                    ...
                    height: 40,
                    borderRadius: 10,
                    background: t.primary_color ?? 'var(--bg-hover)',
                    display: 'flex',
                    alignItems: 'center',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    ...
                    >
                      <Users size={11} />
                      {activeCounts[t.id] ?? 0} active now
                    </span>
                  </div>
                </div>
