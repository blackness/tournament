    ...
    <div className="min-h-screen flex flex-col" style={{ background:'var(--bg-base)' }}>
      ...
      <header style={{ background:'var(--bg-surface)', borderBottom:'1px solid var(--border)', position:'sticky', top:0, zIndex:30 }}>
            ...
            {user ? (
              <>
                {isDirector && (
                  <Link
                    to="/director"
                    style={{ fontSize:13, color:'var(--text-secondary)', padding:'5px 10px', borderRadius:7, textDecoration:'none', display:'flex', alignItems:'center', gap:5 }}
                    className="hover:text-[var(--text-primary)] transition-colors"
                  >
                ...
                {profile?.role === 'admin' && (
                  <Link
                    to="/admin"
                    style={{ fontSize:13, color:'var(--accent)', padding:'5px 10px', borderRadius:7, textDecoration:'none', display:'flex', alignItems:'center', gap:5, border:'1px solid rgba(232,255,71,0.2)', background:'var(--accent-dim)' }}
                  ...
                  {profileOpen && (
                    <div style={{ position:'absolute', right:0, top:'calc(100% + 6px)', width:200, background:'var(--bg-raised)', border:'1px solid var(--border-mid)', borderRadius:12, boxShadow:'0 8px 24px rgba(0,0,0,0.4)', zIndex:50, overflow:'hidden' }}>
                      <div style={{ padding:'12px 14px', borderBottom:'1px solid var(--border)' }}>
                        ...
                        <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:2, overflow:'hidden', textOverflow:'ellipsis' }}>
                          {user.email}
                        </p>
                        {profile?.role && (
        ...
        {menuOpen && (
          <div style={{ borderTop:'1px solid var(--border)', background:'var(--bg-surface)', padding:'12px 16px', display:'flex', flexDirection:'column', gap:2 }}>
            <Link
              to="/tournaments"
              style={{ padding:'10px 8px', fontSize:14, color:'var(--text-secondary)', textDecoration:'none', borderRadius:8 }}
            ...
            {user ? (
              <>
                ...
                <div style={{ borderTop:'1px solid var(--border)', paddingTop:10, marginTop:6 }}>
                  ...
                  <button
                    onClick={handleSignOut}
                    style={{ padding:'10px 8px', fontSize:14, color:'#f87171', background:'transparent', border:'none', cursor:'pointer', width:'100%', textAlign:'left', fontFamily:'inherit' }}
                  >
                    Sign out
                  </button>
                </div>
              ...
              <div style={{ display:'flex', gap:8, paddingTop:10, borderTop:'1px solid var(--border)', marginTop:6 }}>
                <Link to="/login" className="btn btn-ghost btn-sm flex-1 text-center" onClick={() => setMenuOpen(false)}>
                  Sign in
                </Link>
                <Link to="/signup" className="btn btn-primary btn-sm flex-1 text-center" onClick={() => setMenuOpen(false)}>
                  Sign up
                </Link>
              </div>
            )}
      ...
      {isSimulating && (
        <div style={{ background:'rgba(234,179,8,0.1)', borderBottom:'1px solid rgba(234,179,8,0.2)', padding:'8px 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button
            onClick={() => { stopSimulation(); navigate('/admin') }}
