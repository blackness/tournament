...
export function WizardStep5Teams({ onNext, onBack }) {
  ...
  async function handleNext() {
    ...
    console.log('[Step5] handleNext fired', {
      teamsInComponent: teams.length,
      teamsInStore: storeState.teams.length,
      tournamentId,
      activeDivision,
    })
    // Read teams directly from store in case component state is stale
    const liveTeams = storeState.teams
    if (liveTeams.length === 0) {
      setFormError('No teams found. Please add at least one team.')
      return
    }
