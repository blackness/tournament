...
export function WizardStep2Sport({ onNext, onBack, isFirst }) {
  ...
  function validate() {
    if (!sportTemplateId) {
      setError('Please select a sport profile.')
      return false
    }
    return true
  }
  ...
  async function handleNext() {
    ...
    try {
      ...
      setError(err.message || 'Failed to save sport settings.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="section-title">Sport Profile</h2>
        <p className="section-subtitle">
          Choose the sport profile for this tournament. Every profile supports schedule, live scoring, and standings. Some profiles also include sport-specific stats and settings.
        </p>
      </div>
        ...
        <>
          ...
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mb-3">
              Toggle sport-specific stats on or off. Some sport profiles use only basic live scoring and standings, while others support detailed stat tracking.
            </p>

            {stats.length === 0 ? (
              <p className="text-sm text-[var(--text-muted)] italic">
                This sport profile uses basic scoring only. No additional configurable stats are available.
              </p>
            ) : (
              <div className="space-y-2">
                {stats.map(stat => {
                  ...
                  return (
                    <div
                      key={stat.id}
                      className="flex items-center justify-between px-4 py-2.5 rounded-lg border border-[var(--border)] transition-colors"
                    >
                ...
                <div className="flex-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm font-medium text-[var(--text-secondary)]">
                      Enable Spirit of the Game (SOTG)
                    </span>
                  </label>
                  ...
                  {sotgEnabled && sportConfig.sotg_categories && (
                    <div className="mt-2 ml-6">
                      <p className="text-xs text-[var(--text-muted)] mb-1">
                        Categories ({sportConfig.sotg_scale?.min ?? 0}–{sportConfig.sotg_scale?.max ?? 4} scale):
                      </p>
